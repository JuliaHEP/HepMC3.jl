...
class Jljlcxx_Module;
class JlGlobal;

std::shared_ptr<Wrapper> newJlHepMC3_FourVector(jlcxx::Module&);
std::shared_ptr<Wrapper> newJlHepMC3_Units(jlcxx::Module&);
...
std::shared_ptr<Wrapper> newJljlcxx_Module(jlcxx::Module&);
  ...
  if(!std::regex_match(version_str, matches, r)){
    ...
    if(version_int < jllmin || version_int >= jllmax){
      throw std::runtime_error(std::string("Found libcxxwrap_jll version ")
        + version_str + ", while module HepMC3 requires version "
...
JLCXX_MODULE define_julia_module(jlcxx::Module& jlModule){
  ...
  // defined in HepMC3/Units.h:29:10
  ...
  // defined in HepMC3/Units.h:32:10
  ...
  std::vector<std::shared_ptr<Wrapper>> wrappers = {
    ...
    std::shared_ptr<Wrapper>(newJlHepMC3_Units(jlModule)),
    ...
    std::shared_ptr<Wrapper>(newJljlcxx_Module(jlModule)),
    std::shared_ptr<Wrapper>(newJlGlobal(jlModule))
  };
  for(const auto& w: wrappers) w->add_methods();
  add_manual_hepmc3_methods(jlModule);
